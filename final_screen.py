    ...
    while True:    
        if terms_and_condition_screen_window is not None:
            ...
            if terms_and_condition_event is None:
                ...
                data=sg.PopupYesNo('Want to end the installation process',title='ModeFinServer Pvt Ltd',text_color="red",location=(700,300))
...
def home_screen_layout():
    ...
    database_installer_layout,main_institute_wise_key_list,dict_of_institution_wise_result=database_installer_screen()
    ...
    return master_installer_window,main_institute_wise_key_list,dict_of_institution_wise_result
...
def convert_into_words(no):
   inflect_obj = inflect.engine()
   words=inflect_obj.number_to_words(int(no))
   return words    

def value_for_each_institution_result(institute_id):
    text_to_display="Data Insertion  in institute %s"%(institute_id)
    button_value=convert_into_words(institute_id)
    return (text_to_display,button_value)

def institution_result_screen():
    dict_of_institution_wise_result={}
    institution_wise_result_list=[]
    no_of_institute_list=read_no_of_institute_file()
    for institute_id in no_of_institute_list:
        institute_wise_screen=None
        text_to_display,button_value=value_for_each_institution_result(institute_id)
        pop_up_heading="Details for institute %s"%(institute_id)
        pop_text_details="following activity happen in institute %s"%(institute_id)
        dict_of_institution_wise_result.update({button_value:[pop_up_heading,pop_text_details]})
        institute_wise_screen=sg.Image('/home/krishna/Downloads/icons8-task-completed-24.png'),sg.Text(text_to_display,size=(40,1),font='Courier 12',text_color='white'),sg.Button("Show Details",key=button_value)
        institution_wise_result_list.append(institute_wise_screen)
    return institution_wise_result_list,dict_of_institution_wise_result

def databse_result_screen():
    institution_wise_result_list,dict_of_institution_wise_result=institution_result_screen()
    dict_of_institution_wise_result.update({"Tables Creation":["Table Details","Table Creation Details"]})
    dict_of_institution_wise_result.update({"Producer Creation":["Producer Creation","Producer Creation Details"]})
    dict_of_institution_wise_result.update({"View Creation":["View Creation","View Creation Details"]})
    dict_of_institution_wise_result.update({"common_data_insertion":["Common Data Insertion","Common Data Insertion Details",]})
    data_base_result_layout =[
            [sg.Image('/home/krishna/Downloads/icons8-task-completed-24.png'),sg.Text("Table Creation",size=(40,1),font='Courier 12',text_color='white'),sg.Button("Show Details",key="Tables Creation")],
            [sg.Image('/home/krishna/Downloads/icons8-task-completed-24.png'),sg.Text("Producer Creation",size=(40,1),font='Courier 12',text_color='white'),sg.Button("Show Details",key="Producer Creation")],
            [sg.Image('/home/krishna/Downloads/icons8-task-completed-24.png'),sg.Text("View  Creation",size=(40,1),font='Courier 12',text_color='white'),sg.Button("Show Details",key="View Creation")],
            [sg.Image('/home/krishna/Downloads/icons8-task-completed-24.png'),sg.Text("Common Data Insertion",size=(40,1),font='Courier 12',text_color='white'),sg.Button("Show Details",key="common_data_insertion")],
            *[data for data in institution_wise_result_list],
            [sg.Button("OK",key="database_result_event")]]
    return data_base_result_layout,dict_of_institution_wise_result
...
def database_installer_screen():
    ...
    data_base_result_layout,dict_of_institution_wise_result=databse_result_screen()
    major_list=[]       
    no_of_institute_list=read_no_of_institute_file()
    ...
    database_installer_layout = [[sg.TabGroup([[
                ...
                *[data for data in major_list],
                sg.Tab('Database result',data_base_result_layout,key='data_base_result',element_justification="center",disabled=True,visible=False)
                ]])]]    
    return database_installer_layout,main_institute_wise_key_list,dict_of_institution_wise_result
...
def validation_of_installer_type_screen(master_installer_window):
    ...
    while True:
        ...
        if installer_type_event is None:
            ...
            sg.popup("Must Select either Database installer or Application installer",title='ModeFinServer Pvt Ltd',text_color="red",location=(700,300))        
...
def validation_of_db_tool_screen(master_installer_window):
    ...
    while True:
        ...
        if db_tool_event is None:
            ...
            sg.popup("Must Select one database tool",title='ModeFinServer Pvt Ltd',text_color="red",location=(700,300))        
...
def data_base_creditional_verification(data_base_creditional_values):
    ...
    if data_base_creditional_values.get("db_name")=='':
        sg.popup("Database name  field cannot be empty",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,400))
        logger.warning("Database name cannot be empty")
    elif data_base_creditional_values.get("localhost")=='': 
        sg.popup("Host name field cannot be empty",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,400))
        logger.warning("Host name field cannot be empty")
    elif data_base_creditional_values.get("username")=='': 
        sg.popup("Username field cannot be empty",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,400))
        logger.warning("Username field cannot be empty")
    elif data_base_creditional_values.get("password")=='': 
        sg.popup("Password field cannot be empty",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,400))  
        ...
        if connection_messages=="Success":
            ...
            sg.popup("wrong user_id or password Please provide Correct user_id and password",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,400)) 
            logger.warning("wrong user_id or password Please provide Correct user_id and password")
        elif table_existance_single==True:
            sg.popup("DataBase already exist Please change the database name",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,400)) 
            logger.warning("DataBase already exist Please change the database name")
        elif host_error_single==True:
            sg.popup("Name or service unknown Please Correct host address",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,400)) 
            logger.warning("Name or service not known Please Correct host address")
        else:
            sg.popup(connection_messages,title="'ModeFinServer Pvt Ltd'",text_color="red",location=(600,400)) 
...
def validation_of_institute_screen(master_installer_window,individual_inst_data):
    ...
    while True:
        ...
        if institute_event is None:
            ...
            sg.popup("Bank name can't be empty provide Bank name",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,400)) 
            logger.warning("Bank name can't be empty provide Bank name")
        
        elif institute_values.get(individual_inst_data[2])=='':
            sg.popup("Location name  can't be empty provide Location name",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,400)) 
            logger.warning("Location name  can't be empty provide Location name")

        elif institute_values.get(individual_inst_data[3])=='':
            sg.popup("Intergration Folder Name can't be empty\n provide name for Intergration Folder Name",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,400)) 
            ...
            sg.popup("must select atleast one language",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,400)) 
...
def validation_of_application_type_screen(master_installer_window):
    ...
    while True:
        ...
        if application_Server_type_event is None:
            ...
            sg.popup("Must Select either Apache Tomcat or Oracle WebLogic",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,300))        
...
def validation_of_application_home_directory_screen(master_installer_window):
    ...
    while True:
        ...
        if home_directory_event is None:
            ...
            sg.popup("Home Directory field cannot be empty",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,300))
            ...
            try:
                ...
                if str(err).find(file_not_finding_message)!=-1:
                    ...
                    logger.warning(alert_message_for_changin_home_directory)
                    sg.popup(alert_message_for_changin_home_directory,title='ModeFinServer Pvt Ltd',text_color="red",location=(600,300))
                else:
                    sg.popup(err,title='ModeFinServer Pvt Ltd',text_color="red",location=(600,300))
                    logger.error(err)
                    break 
...
def customise_application_input_screen_verification(master_installer_window):
    ...
    while True:
        ...
        if application_customise_input_event is None:
            ...
            sg.popup("Database tool name  field cannot be empty",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,350))
            logger.warning("Database tool name  field cannot be empty")
        elif application_customise_input_values.get("Database_Name")=='': 
            sg.popup("Database name  field cannot be empty",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,350))
            logger.warning("Database name  field cannot be empty")
        elif application_customise_input_values.get("Database_User_Name")=='': 
            sg.popup("Database user name  field cannot be empty",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,350))
            logger.warning("Database user name  field cannot be empty")
        elif application_customise_input_values.get("Database_Pass_Word")=='': 
            sg.popup("Database password field cannot be empty",title='ModeFinServer Pvt Ltd',text_color="red",location=(600,350))  
...
def performing_apache_tomcat_task(master_installer_window):
    ...
    if (home_directory_path_message=="Success") and (home_directory_path is not None):
            ...
            if zip_extracting_message=="Success":
                ...
                sg.popup(zip_extracting_message,title='ModeFinServer Pvt Ltd',text_color="red",location=(700,300))
                apache_tomcat_task="Fail"    
...
def performing_database_installation_task(master_installer_window,main_institute_wise_key_list,dict_of_institution_wise_result):
    ...
    if db_tool_event=="database_tool_event":
        if db_tool_values=="MYSQL":
            message_for_processing_of_mysql_task=performing_mysql_database_installer_task(master_installer_window,main_institute_wise_key_list,dict_of_institution_wise_result)
            ...
            sg.popup("ORACLE database is not yet not supported",title='ModeFinServer Pvt Ltd',text_color="red",location=(700,300))        
        elif db_tool_values=="MSSQL": 
            sg.popup("MSSQL database is not yet not supported",title='ModeFinServer Pvt Ltd',text_color="red",location=(700,300))        
        else: 
            sg.popup("POSTGRESS database is not yet not supported",title='ModeFinServer Pvt Ltd',text_color="red",location=(700,300))                 
    ...
    try:
        ...
        if db_installer_zip_file_extract_message=="Success":
            ...
            if terms_and_condition_event=="Accept":
                master_installer_window,main_institute_wise_key_list,dict_of_institution_wise_result=home_screen_layout()
                ...
                if installer_type_event=="installer_type_button":
                    if installer_type_values=="Database Installer":
                        message_for_processing_of_database_installation_task=performing_database_installation_task(master_installer_window,main_institute_wise_key_list,dict_of_institution_wise_result)
            ...
            sg.popup(db_installer_zip_file_extract_message,title="'ModeFinServer Pvt Ltd'",text_color="red",location=(700,300))
            master_installer_window.close()
    except Exception as err:
            logger.error("following error raised %s",err) 

    #################_mysql_database_installer_task#############################
def performing_mysql_database_installer_task(master_installer_window,main_institute_wise_key_list,dict_of_institution_wise_result):
                        ...
                        if excute_global_data_message=="Success":
                            ...
                            if (connected_message=="Success") and (connection is not None) and (cursor is not None):
                                ...
                                if institution_handler_message=="Success":
                                    ...
                                    cursor.close()
                                    connection.close()
                                    database_installer_result_message=validation_of_database_installer_result(master_installer_window,dict_of_institution_wise_result,gobal_result_dict)
                                    if database_installer_result_message=="Success":
                                        message_for_processing_of_mysql_task=database_installer_result_message
    return message_for_processing_of_mysql_task

def prepare_result_message(gobal_result_dict,database_result_event):
    result_message=''
    list_of_result_message=gobal_result_dict.get(database_result_event)
    if list_of_result_message is not None:
        for sub_list_first in list_of_result_message:
            if len(sub_list_first)>0:
                mirco_message='\n'
                mirco_message=mirco_message.join(sub_list_first)
                result_message=result_message+'\n'+mirco_message
    return (result_message)
        
def validation_of_database_installer_result(master_installer_window,dict_of_institution_wise_result,gobal_result_dict): 
    database_installer_result_message=None
    master_installer_window.FindElement('data_base_result').Update(visible=True)
    master_installer_window.FindElement('data_base_result').Update(disabled=False)
    master_installer_window.FindElement('data_base_result').select()
    database_installer_result_message="Success"
    while True:
        database_result_event,database_result_values = master_installer_window.read() 
        if(database_result_event=="database_result_event"):
            database_installer_result_message="Success"
            break
        elif(database_result_event is None):
            database_installer_result_message="Fail"
            break
        else:
            result_message=prepare_result_message(gobal_result_dict,database_result_event)
            if len(result_message)>0:
                result_message=result_message.strip()
                sg.popup_scrolled(result_message,title='ModeFinServer Pvt Ltd',size=(65,15),font='Courier 12',text_color='black',location=(400,200))  
            else:
                result_message="Nothing is created or inserted"
                sg.popup_scrolled(result_message,title='ModeFinServer Pvt Ltd',size=(65,15),font='Courier 12',text_color='black',location=(400,200))  
    return database_installer_result_message
...
def institution_handler(connection,cursor,main_institute_wise_key_list,list_of_dict_for_each_inst):
    ...
    try:
        ...
        sg.Popup(err,title="'ModeFinServer Pvt Ltd'",text_color="red",location=(700,300))
...
def place_holder_replacer(sub_folder_name,sub_list,institute_values,cursor):
    main_list_of_individual_institute_data=[]
    ...
    status_data="Insert data in institute %s"%(str(sub_list[0]))
    ...
    for institutional_file in (os.listdir(Path(institutional_folder_path))):
        sub_list_of_individual_institutes_data=[]
        ...
        with open(Path(updated_file), "w") as out_put_file_obj:
            ...
            log_event_message,sub_result_list=sql_excution(updated_sql_stmt,cursor,excuted_file_name,status_data,sub_list_of_individual_institutes_data)
            main_list_of_individual_institute_data.append(sub_result_list)
    ...
    words=convert_into_words(sub_list[0])
    gobal_result_dict.update({words:main_list_of_individual_institute_data})
...
def object_creation(sql_statement,cursor,excuted_file_name,status_data):
    ...
    for data in sql_statement.split("\n"): 
        ...
        if create_file_path:
            ...
            log_event_message,sub_result_list=sub_object_creation(create_sql_statement,excuted_file_name,cursor,status_data,main_sub_list_name)
            main_list_name.append(sub_result_list)
            if log_event_message is None:
                processing_windows_screen_message="Nothing is excuted or created from %s file"%(excuted_file_name)
                logger.info(processing_windows_screen_message)
            else:
                processing_windows_screen_message="excuting %s \n %s "%(excuted_file_name,log_event_message)
            some_data="out of %s %s is excuting"%(str(max_len_for_processing_window),str(temp_data))
            output_window_screen['output_text'].update(some_data)
            progress_bar.update_bar(temp_data,(max_len_for_processing_window+1))
            print(processing_windows_screen_message)
            temp_data=temp_data+1
    gobal_result_dict.update({status_data:main_list_name})
    output_window_screen.close()

def sub_object_creation(create_sql_statement,excuted_file_name,cursor,status_data,main_sub_list_name):
    ...
    if create_sql_statement.find("DELIMITER")!=-1:
        ...
        log_event_message,sub_result_list=sql_excution(create_sql_statement,cursor,excuted_file_name,status_data,main_sub_list_name)
    else:    
        log_event_message,sub_result_list=sql_excution(create_sql_statement,cursor,excuted_file_name,status_data,main_sub_list_name)
    return log_event_message,sub_result_list
...
def excute_global_data(global_directory,cursor):
    list_of_all_excuted_gobal_data=[]
    excute_global_data_message=None
    try:
        status_data="Insert data from gobal folder"
        ...
        for global_file_path in (os.listdir(global_directory)):
            event,values = output_window_screen.read(timeout=1000)
            list_of_sub_excuted_gobal_data=[]
            required_file=str(global_directory)+"/"+global_file_path
            global_sql_statement,excuted_file_name=read_sql_file(required_file)
            log_event_message,sub_result_list=sql_excution(global_sql_statement,cursor,excuted_file_name,status_data,list_of_sub_excuted_gobal_data) 
            list_of_all_excuted_gobal_data.append(sub_result_list)
            processing_windows_screen_message="excuting %s \n %s "%(excuted_file_name,log_event_message)
            logger.info(processing_windows_screen_message)   
            some_data="out of %s %s is excuting"%(str(max_len_for_processing_window),str(temp_data))
            output_window_screen['output_text'].update(some_data)
            ...
            temp_data=temp_data+1
        output_window_screen.close()
        gobal_result_dict.update({"common_data_insertion":list_of_all_excuted_gobal_data})
...
def sql_excution(sql_statement,cursor,excuted_file_name,status_data,sub_result_list):
        ...
        for result in cursor.execute(sql_statement,multi=True):
            try:
                ...
                if table_name:
                    ...
                    effected_row="Number of rows affected by statement %s"%(result.rowcount)
                    logger.info(effected_row)
                    sub_result_list.append(log_event_message)
                    sub_result_list.append(effected_row)
...
def create_log_message_for_event_excution(status_data,table_name):
    ...
    elif  status_data.find("Insert data")!=-1:
